...
| ♻️🔥 **再点火**（最重視） | 全盛期に 1h 出来高 **$300K 以上**を記録し、いまその **50% 以下**に冷え、レンジ上限を **+6% 以上**上抜け | `♻️🔥 再点火 ｜ 出来高 低/中/高` |
...
いちばん価値があるのは「ローンチで $2〜3M/h を出し、$1M/h に落ち、下がってレンジを組み、そこから上がる」型です。
**過去に大きな出来高を記録したこと自体が、実需の関心を集めた証拠**になります。無名の銘柄が偶然動いたのとは違うため、
レンジ抜けのしきい値を +12% → **+6%** まで下げても誤検知が増えません。つまり**より早く通知できます**。

そのために各銘柄の全盛期（最大 1h 出来高・最高値・その時刻）を `pairs` テーブルに永続保存しています。
スナップショットの保持期間を過ぎても消えないので、何日前の全盛期でも判定に使えます。
さらに全盛期が大きい銘柄は、いま静かでも `PRIORITY_POLL_INTERVAL_SEC`（既定 45 秒）ごとに見張ります。
再点火はこうした銘柄から起きるため、dormant の 4 分間隔では遅すぎるからです。

同じ値動きでの比較:

```
レンジ上限 $0.00118 を抜けていく過程        元大物      履歴なし
  $0.001240 (レンジ + 5.1%)                —          —
  $0.001251 (レンジ + 6.0%)              🔔 再点火     —
  $0.001322 (レンジ +12.0%)              🔔 再点火    🔔 検知
```
